...
| `searchLogs` | 搜索日志表 | 记录用户搜索行为，用于数据分析 |
...
| `city` | String | 否 | 搜索城市 |
| `hasResult` | Boolean | 是 | 是否有搜索结果 |
| `resultCount` | Number | 是 | 结果数量 |
| `createTime` | Date | 是 | 搜索时间 |

**隐私说明**：
- 不记录用户身份信息（OpenID、昵称等）
- 仅记录搜索行为本身
- 建议定期清理，数据保留30天
