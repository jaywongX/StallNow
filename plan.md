...
"小摊记"是一款地摊分布与发现工具小程序，旨在推广家乡美食文化，方便游客发现地摊、帮助商贩展示信息。
...
索引：
- { createTime: -1 }           # 按时间倒序
- { keyword: 1, createTime: -1 } # 按关键词统计
```

**实现完成**：
- ✅ 新增 `searchLogs` 集合
- ✅ 在 `getStalls` 云函数中，当 `keyword` 参数存在时异步记录日志
- ⬜ 后台管理添加「搜索热词」统计页面（后续优化）
...
**问题**：`lastConfirmedAt` 字段用于控制摊位可信度状态显示，但目前只有管理员审核时更新，商贩无法自行更新，导致摊位状态容易变为"信息过期"

**解决方案**：在"我的摊位"页面新增每日签到功能，让商贩可以自行确认摊位有效性

#### 7.1 功能设计

**签到按钮位置**：摊位卡片顶部，状态标签旁边

**签到规则**：
- 每天可签到一次
- 签到后更新 `lastConfirmedAt` 为当前时间
- 签到后 `reliability` 重置为 0（近期确认）
- 显示今日是否已签到状态
...
```javascript
{
  lastConfirmedAt: Date,      // 最后确认时间（签到时更新）
  lastCheckInDate: String,    // 最后签到日期（YYYY-MM-DD格式，用于判断今日是否已签到）
  reliability: Number         // 可信度状态：签到后重置为 0
}
```
...
1. 验证用户是否为摊位所有者
2. 检查今日是否已签到
3. 更新 `lastConfirmedAt`、`lastCheckInDate`、`reliability`
4. 返回签到结果

**返回格式**：
```javascript
// 成功
{ code: 0, data: { alreadyCheckIn: false, message: '签到成功' } }

// 今日已签到
{ code: 0, data: { alreadyCheckIn: true, message: '今日已签到' } }

// 失败
{ code: -1, message: '无权限操作' }
```
...
**vendor-manage.js**：
- 新增 `onCheckIn()` 方法处理签到
- 新增 `isTodayChecked` 数据字段
- `updateStallStatus()` 方法增加今日是否已签到判断
...
```
💡 温馨提示
建议每天签到一次，这样用户就知道你的摊位还在营业中。
签到后，摊位状态会显示为"今日可能在"，更容易被找到哦～
```
...
|--------|------|------------|----------|----------|
| **P0** | **游客推荐摊位功能** | **2天** | **v2.0** | **用户明确需求** |
| **P0** | **认领审核功能** | **3天** | **v2.0** | **用户明确需求** |
| **P0** | **商贩每日签到功能** | **1天** | **v2.0** | **用户明确需求** |
...
目前入驻申请功能只能由**管理员代录入**或**摊主自己申请**。普通游客（用户）如果发现一个好摊位，没有便捷的途径推荐给平台。这限制了内容来源，也让热心用户无法参与平台建设。

### 目标用户

- **普通游客**：在路边发现好吃的摊位，想分享给更多人
- **本地居民**：熟悉周边环境，愿意贡献摊位信息
- **尝鲜用户**：刚体验完某个摊位，想帮助摊主增加曝光
...
|--------|----------|----------|
| **填写者身份** | 普通用户/游客 | 摊主本人 |
| **必填信息** | 名称、位置、分类、照片 | 完整信息（联系方式、营业时间等） |
| **选填信息** | 外观描述、出摊时间 | 详细联系方式、商家名称 |
| **审核严格度** | 较宽松（验证位置合理性） | 标准（验证身份真实性） |
| **创建后权限** | 无管理权 | 获得摊位管理权 |
| **createdBy** | `user_recommend` | `vendor_self` |

### 功能设计

#### 1. 入口位置

在个人中心页面（`profile`）增加"推荐摊位"入口：
...
**设计理念**：让游客能在 1 分钟内完成推荐，不增加心理负担。

**Step 1｜你在哪发现了这个摊位？**
- **摊位名称**（必填）：placeholder "例如：路口烧烤摊"
- **定位位置**（必填）：点击选择地图位置
- **大致描述**（选填）：placeholder "例如：在XX路口，有个红色棚子"

**Step 2｜这是什么类型的摊位？**
- **商品类型**（单选）：小吃/烧烤、水果/零食、手工/饰品、玩具/文创、其他
- **商品标签**（多选，最多3个）：烤串、煎饼、奶茶等快捷标签
...
游客推荐的摊位通过后，摊主可以通过以下方式认领：

1. **扫码认领**：摊主扫描二维码，进入认领申请页面
2. **搜索认领**：摊主在小程序内搜索自己摊位，点击"认领"

认领需要提交认证信息，通过审核后获得管理权。
...
```javascript
{
  ...
  // 创建方式扩展
  createdBy: 'user_recommend',  // 新增：user_recommend（游客推荐）
                                // 原有：vendor_self（摊主申请）
                                // 原有：admin_proxy（管理员代录）
  
  // 推荐人信息（游客推荐时填写）
  recommendedBy: String,        // 推荐用户ID
  recommendedAt: Date,          // 推荐时间
  
  // 认领状态（游客推荐默认为待认领）
  claimStatus: 'unclaimed',     // unclaimed / pending / claimed / rejected
  claimedBy: '',               // 认领摊主用户ID
  claimedAt: Date              // 认领时间
}
```
...
1. **`pages/recommend-stall/`** - 游客推荐摊位表单
   - `recommend-stall.js` - 页面逻辑
   - `recommend-stall.wxml` - 页面结构
   - `recommend-stall.wxss` - 页面样式
   - `recommend-stall.json` - 页面配置

2. **`pages/recommend-success/`** - 推荐成功页面（可选）
   - 显示提交成功提示
   - 引导用户继续浏览或查看"我的推荐"
...
**`submitStallRecommendation`** - 提交摊位推荐

```javascript
/**
 * 提交游客推荐的摊位
 * @param {Object} data - 推荐数据
 * @param {String} data.displayName - 摊位名称
 * @param {Object} data.location - 地理位置 {latitude, longitude, address}
 * @param {String} data.categoryId - 分类ID
 * @param {Array} data.goodsTags - 商品标签
 * @param {Array} data.images - 图片URL数组
 * @param {Array} data.scheduleTypes - 出摊时间
 * @returns {Object} {code, data: {stallId}, message}
 */
```

**实现逻辑**：
1. 验证用户已登录
2. 创建 stalls 记录，status=0（待审核）
3. 设置 createdBy='user_recommend'
4. 设置 recommendedBy=当前用户ID
5. 返回摊位ID

### 审核后台扩展

在管理员后台（`admin`）增加"游客推荐"标签页：
...
- 游客推荐同样需遵守图片合规要求（无价格、收款码）
- 明确提示：游客推荐不等于摊主入驻，摊主后续可认领
- 免责声明：信息由用户志愿提供，仅供参考
...
| 步骤 | 任务 | 文件 | 预计时间 |
|-----|------|------|---------|
| 1 | 创建推荐表单页面 | `pages/recommend-stall/` | 0.5天 |
| 2 | 实现提交云函数 | `submitStallRecommendation` | 0.3天 |
| 3 | 更新个人中心入口 | `profile.wxml`, `profile.js` | 0.1天 |
| 4 | 扩展管理员后台 | `admin.js`, `admin.wxml` | 0.5天 |
| 5 | 测试与优化 | 全流程测试 | 0.6天 |
